...
def test_denoise_tv_chambolle_multichannel():
    ...
    astro3 = np.tile(astroT[:64, :64, np.newaxis, :], [1, 1, 2, 1])
    # modify along tiled dimension to give non-zero gradient on 3rd axis
    astro3[:, :, 0, :] = 2*astro3[:, :, 0, :]
    astro3 = torch.tensor(astro3, dtype = torch.float32)
    denoised0 = denoise_tv_chambolle_torch(astro3[..., 0], weight=0.1)
    denoised = denoise_tv_chambolle_torch(astro3, weight=0.1,
                                                multichannel=True)
    assert_equal(denoised[..., 0].numpy(), denoised0.numpy())
