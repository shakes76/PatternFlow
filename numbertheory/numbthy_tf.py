...
def cond0_is_prime(n):
  ...
  against 2, 3 and 5"""
  return tf.math.logical_and(tf.math.logical_and(isprimeE(n,2), isprimeE(n,3)), isprimeE(n,5))
...
def cond0_factorPR(n, true0, g):
  result = tf.cond(tf.equal(g,n), lambda:(n, tf.constant(0), g), lambda:(n, tf.constant(1), g))
  return result
              
def while0_body_factorPR(additive, slow, fast, numsteps, i, n, true0, g):
  ...
  n, true0, g = tf.cond(tf.equal(g,1), lambda:(n, true0, g), lambda:cond0_factorPR(n, true0, g))
  return additive, slow, fast, numsteps, i, n, true0, g
  
def while0_cond_factorPR(additive, slow, fast, numsteps, i, n, true0, g):
  return tf.cond(tf.equal(true0,3), lambda:tf.less(i,numsteps), lambda:tf.equal(0,1))

def while1_body_factorPR(numsteps, additive, n, true0, g):
  fast = tf.constant(1)
  slow = tf.constant(1)
  i = tf.constant(1)
  additive, slow, fast, numsteps, i, n, true0, g = tf.while_loop(while0_cond_factorPR, 
                         while0_body_factorPR, 
                         [additive, slow, fast, numsteps, i, n, true0, g])
  additive = tf.math.add(additive,1)
  return numsteps, additive, n, true0, g

def while1_cond_factorPR(numsteps, additive, n, true0, g):
  return tf.cond(tf.equal(true0,3), lambda:tf.less(additive,5), lambda:tf.equal(0,1))
  
def factorPR(n):
  numsteps = tf.math.multiply(tf.constant(2.0),tf.math.floor(tf.math.sqrt(tf.dtypes.cast(tf.constant(10),tf.float32))))
  numsteps = tf.dtypes.cast(numsteps,tf.int32)                            
  additive = tf.constant(1)
  true0 = tf.constant(3)
  g = tf.constant(0)
  result = tf.while_loop(while1_cond_factorPR, while1_body_factorPR, [numsteps, additive, n, true0, g])
  return tf.cond(tf.equal(result[3],1), lambda:result[4], lambda:tf.constant(1))
