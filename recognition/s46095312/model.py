...
def dice_metric(y_true, y_pred, min_delta=1e-6):
...
def soft_dice_loss(y_true, y_pred, min_delta=1e-6):
    # calculate sums over the x and y axis of each label frame in the batch
    ...
    return 1. - K.mean((num + min_delta) / (denom + min_delta))
...
def unet_model(input_size=(256, 256, 1)):
    ...
    conv_19 = layers.Conv2D(1, (1, 1), activation='softmax', padding='same')(conv_18)

    model = Model(input_layer, conv_19)
    model.compile(optimizer=Adam(learning_rate=1e-4), loss=soft_dice_loss, metrics=['dice_metric'])
    return model
...
class UNet:
    ...
    def fit(self, train_input, train_labels, validation, validation_labels, epochs):
        return self.unet.fit(train_input, train_labels, validation_data=(validation, validation_labels),
