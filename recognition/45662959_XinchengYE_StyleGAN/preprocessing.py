...
if __name__ == '__main__':
    with zipfile.ZipFile('AKOA_Analysis.zip', 'r') as zip_ref:
        zip_ref.extractall('AKOA_Analysis')
    imgset = datasets.ImageFolder('AKOA_Analysis')
    with lmdb.open('AKOA_PRE', map_size=1024 ** 4, readahead=False) as env:
        with env.begin(write=True) as txn:
            prepare(txn, imgset, 8)
