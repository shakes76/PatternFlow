"""
Driver script to show load model weights and prediction example
...
def main():
    ...
    model_path = ROOT_PATH + "\Models\Imporoved_Unet.h5"
    ...
    _, _, X_test, _, _, y_test = train_val_test_split(image_path, mask_path, new_imageshape, new_imageshape, split_ratio, randomstate=42)

    # Construct Improved Unet model
    print("\nConstructing model...")
    model = SegModel((new_imageshape, new_imageshape, 3), random_seed=42, model="Improved_Unet")
    # Load the Improved Unet model weights
