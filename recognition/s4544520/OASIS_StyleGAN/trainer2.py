...
class Trainer:
    ...
    def train(self):
        ...
        for stage in range(1, num_stage + 1):
            ...
            for epoch in range(stage_epoch):  # each stage should be trained some epochs
...
# from transforms import RatioCenterCrop, RandomRotate
...
#         self.init_size = arg.init_size
#         self.output_size = arg.size
#         self.stage_epoch = arg.unit_epoch
#         self.lr = arg.lr
#         self.batch_size = arg.batch_size
#         # network init:
#         self.G = Generator(nc=self.nc, nz=self.nz, size=self.output_size).to(self.device)
#         self.D = Discriminator(nc=self.nc, nz=self.nz, size=self.output_size).to(self.device)
#         self.G_EMA = copy.deepcopy(self.G).to('cpu')
#         self.lambda_drift = arg.lambda_drift


#         self.g_optim = optim.Adam(self.G.parameters(), lr=self.lr, betas=(0, 0.99), eps=1e-8, weight_decay=0.)
#         self.d_optim = optim.Adam(self.D.parameters(), lr=self.lr, betas=(0, 0.99), eps=1e-8, weight_decay=0.)
#         self.writer = SummaryWriter(self.output_size)
...
#                                                       shuffle=True, pin_memory=True, drop_last=True
#                                                       )
#         self.counts = self.stage_epoch * len(self.dataloader)
...
#                                   grad_outputs=torch.ones_like(disc_interpolates).to(self.device), create_graph=True,
...
#         # real label D
#         real_d = self.D(label)
#         real_d_loss = real_d.mean().mul(-1.)
#         loss_real_drift = real_d.pow(2.).mean()
...
#         D_loss = real_d_loss + loss_d_fake +self.lambda_drift * loss_real_drift+10.0 * gp
#         W_dist = real_d_loss.item() + real_d_loss.item()
#         D_loss.backward()
#         self.d_optim.step()
...
#         fake = self.G(z)
#         fake_d = self.D(fake)
#         G_loss = fake_d.mean().mul(-1.)
#         G_loss = G_loss.to(self.device)
#         G_loss.backward()
#         self.g_optim.step()
#         return G_loss.item(), D_loss.item(), W_dist

#     def progressive_update(self, counter):
#         dfactor = 1. / self.counts
#         if counter < self.counts:
#             self.G.model.residual_module.update_alpha(dfactor)
#             self.D.model.residual_module.update_alpha(dfactor)
#             self.G_EMA.model.residual_module.update_alpha(dfactor)
...
#     def train(self):
#         x_axis = 0
#         num_stage = int(math.log2(self.output_size) - 1)
#         for stage in range(1, num_stage + 1):
#             # stage's img size
#             current_stage_size = int(4 * pow(2, (stage - 1)))
#             eps = self.stage_epoch
#             # dataset setting to change different size
#             self.dataset = Trainset(current_stage_size)
#             # length = len(self.dataset)
#             # train_size, validate_size = int(24), int(length - 24)
#             # self.dataset, validate_set = torch.utils.data.random_split(self.dataset, [train_size, validate_size])
#             self.dataloader = torch.utils.data.DataLoader(self.dataset, batch_size=self.batch_size,num_workers=8,
...
#             count = 0
#             for epoch in range(eps):  # each stage should be trained some epochs
#                 for index, current_stage_label in enumerate(self.dataloader):  # this stage 's img_dataloder
#                     # concat_factor = self.progressive_update(stage, updated_count)
#                     concat_factor = 0
#                     img = current_stage_label

#                     if stage != 1:
#                         concat_factor = self.progressive_update(count)
#                         self.writer.add_scalar('archive/current_alpha', concat_factor, x_axis)
#                         last_stage_size_img = F.avg_pool2d(img, 2)
#                         last_stage_for_current_stage = F.interpolate(last_stage_size_img, scale_factor=2.,
#                                                                      mode='nearest')
#                         label = (1 - concat_factor) * last_stage_for_current_stage + concat_factor * img
#                         label = label.to(self.device)
...
#                         print(
#                             "Stage {}/{} epoch {}/{} G_loss {:4f} D_loss{:4f} W_dis {:4f}" \
#                                 .format(stage, num_stage, epoch + 1, eps, G_loss, D_loss, W_dis)
#                         )
#                         self.writer.add_scalar('train/G_loss', G_loss, x_axis)
#                         self.writer.add_scalar('train/D_loss', D_loss, x_axis)
#                         self.writer.add_scalar('train/W_dis', W_dis, x_axis)
...
#                     fixed_z = torch.FloatTensor(self.batch_size, 512, 4, 4).normal_(0.0, 1.0)
#                     self.G_EMA.eval()
#                     fake = self.G_EMA(fixed_z)
#                     generate = utils.make_grid(fake, nrow=4, normalize=True, scale_each=True)
#                     self.writer.add_image('stage_{}/fake'.format(stage), generate, epoch)
...
#                         'G': self.G.state_dict()
#                     }
#                     torch.save(checkpoint, os.path.join("./logs", 'stage{}.tar'.format(stage)))
