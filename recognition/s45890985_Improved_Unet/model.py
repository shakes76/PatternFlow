...
def improved_unet(width, heigh, channels):
    inputs = layers.Input((width,height,channels))

    #encoding path
    en1 = layers.Conv2D(16, (3, 3), padding='same')(inputs)
    ...
    outputs = activations.softmax(seg3)
...
def encoder_block(input, conv_depth):
    en = layers.Conv2D(conv_depth, (3, 3), padding='same')(input)
