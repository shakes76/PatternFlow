...
from tensorflow.keras.layers import Input, Reshape, Conv2DTranspose, Flatten, Dense
...
training_ds = image_file_paths.map(parse_image, num_parallel_calls=AUTOTUNE).cache().batch(BATCH_SIZE)
...
def generator_network(input_shape):
    ...
    dense = layers.FullyConnected(input, 8*8*512)
    reshape = Reshape((8, 8, 512))(dense)
    
    conv1 = layers.Generator_Norm_Conv2DTranspose(reshape, filters=512)
    ...
    # todo make this a function in the layers module
    conv5 = Conv2DTranspose(1, kernel_size=(3,3), strides=(2,2), padding='same', use_bias=True, activation='tanh')(conv4)
...
def discriminator_network(input_shape):
    ...
    '''
    input = Input(shape=input_shape)
    conv1 = layers.Discriminator_Norm_Dilated_Conv2D(input, filters=64, dropout=0.15)
    conv2 = layers.Discriminator_Norm_Dilated_Conv2D(conv1, filters=128, dropout=0.15)
    conv3 = layers.Discriminator_Norm_Dilated_Conv2D(conv2, filters=256, dropout=0.15)
    conv4 = layers.Discriminator_Norm_Dilated_Conv2D(conv3, filters=512, dropout=0.1)
...
def train(dataset, epochs):
    ...
    for epoch in range(1, epochs+1):
        ...
        for image_batch in dataset:
            generator_loss, discriminator_loss = train_step_dcgan(image_batch)
...
#training
gen_losses, disc_losses = train(training_ds, EPOCHS)
