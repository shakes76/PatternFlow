...
def split_reim(array):
    ...
    Args:
      array(complex): Array shape (batch_size, N, N) or (batch_size, N, N, 1)
    Returns:
      split_array(float): Array shape (batch_size, N, N, 2) 
      containing the real part and the imaginary part on different channels
    """
...
def join_reim(array):
    """the real and imaginary channels of a matrix are joined to single complex-valued matrix.
    Args:
      array(float): array shape (batch_size, N, N, 2)
    Returns:
      joined_array(complex): An array of complex values of shape (batch_size, N, N, 1)
    """
