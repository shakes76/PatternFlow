...
"""
def denormalise(data: np.ndarray, mean: float) -> np.ndarray:
    """
    ...
    Args:
        data (np.ndarray): normalised data
...
def create_image(data: np.ndarray, name: str = None, output_folder: str = "output/") -> Image:
    """
    Creates an new PNG image from a generated data matrix.
    Saves image to output folder if a name is specified

    Args:
        data (np.ndarray): uint8 single channel matrix of greyscale image pixel intensities
