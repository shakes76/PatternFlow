...
class StyleGAN():
    ...
    def __init__(self, output_res: int = 256, start_res: int = 4, latent_dim: int = 512, existing_model_folder: str = None) -> None:
        ...
        if existing_model_folder is not None:
            ...
            self._make_model(output_res, start_res, latent_dim, 0, 0, None, None)
            

    def _make_model(self, output_res: int, start_res: int, latent_dim: int, trained_epochs: int, trained_mean:int, generator: tf.keras.Model, discriminator: tf.keras.Model) -> None:
        """
        Constructs a StyleGAN, setting the specified parameters, and linking a generator and discriminator

        Args:
            output_res (int): side length of images to be output by generator #TODO power of 2 suggestion
            start_res (int): side length of the constant tensor to which the generator upscales and convolves into an image
            latent_dim (int): dimension of the latent space which the generator draws from. This also specifies the dimension of the adaIN parameter tensors and thus the number of filters per generator block. #TODO add this per docstring
            trained_epochs (int): Number of epochs to which the constructed model has been previously trained.
            trained_mean (int): The mean used in normalising the data this model has been trained on
            generator (tf.keras.Model): Model specifiying the architecture and weights to use for the generator. Set this to None to generate a new generator architecture with newly initialised weights.
            discriminator (tf.keras.Model): Model specifiying the architecture and weights to use for the discriminator. Set this to None to generate a new discriminator architecture with newly initialised weights.
        """
        ...
        self._generator_base = tf.ones(shape = (1,self._start_res//2,self._start_res//2,self._latent_dim)) #Start generation with a base of 1's instead of 0's as taking a 'subtractive' generation approach appeared to produce more stable training behaviour than an 'additive' #TODO line length
    ...
    def get_discriminator(self) -> tf.keras.Model:
        ...
        x = tf.keras.layers.Dense(1, name = "Discriminate")(x) #Final decision. the higher the value, the more 'real' an image
    ...
    def __call__(self, inputs: list[np.array]) -> np.array:
        ...
        Args:
            inputs (list[np.array]): Set of inputs for the generator: Latent vector followed by the ordered set of pairs of noise inputs.
    ...
    def save_model(self, folder: str) -> None:
        """
        Saves styleGAN to a targetted folder. Separately saves the keras model containing the discriminator and generator, with their respective weights.
        This allows training to continue to be split after a reload, with these two components being able to be relinked with weights carrying over upon loading. 
        Also saves the attributes used surrounding these two models, which are useful in designing/conducting training paradigm.
        ...
        self._discriminator.save(folder + "discriminator")
        self._generator.save(folder + "generator")

    def load_model(self, folder :str) -> None:
        """
        Loads a styleGAN from a specified folder. Model will posess weights and relevent details of any prior training conducted on it.
        ...
        self._make_model(int(params[0]),int(params[1]),int(params[2]),int(params[3]),float(params[4]),generator,discriminator)
    ...
    def track_mean(self, mean: float) -> None:
        ...
        a separate instance of the data loader used to retrieve the training data.

        Args:
            mean (float): mean used to center the data used to train styleGAN instance
        """
