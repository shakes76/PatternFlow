...
def fit(style_GAN: modules.StyleGAN, images: np.array, batch_size: int, epochs: int, training_history_location: str, image_sample_count:int = 5, image_sample_output:str = "output/", model_output:str = "model") -> None:
    """
    ...
    constant_generator_base = tf.zeros(shape = (batch_size,style_GAN._start_res//2,style_GAN._start_res//2,style_GAN._latent_dim))
    ...
    for e in range(epochs):
        ...
        for b in range(num_batches):
            ...
            fake_images = style_GAN._generator(GANutils.random_generator_inputs(batch_size,style_GAN._latent_dim,style_GAN._start_res,style_GAN._output_res) + [constant_generator_base])
            ...
            gl, ga = style_GAN._gan.train_on_batch(GANutils.random_generator_inputs(batch_size,style_GAN._latent_dim,style_GAN._start_res,style_GAN._output_res) + [constant_generator_base], real_labels)
