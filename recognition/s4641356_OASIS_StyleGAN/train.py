...
def fit(style_GAN: modules.StyleGAN, data_loader: dataset.OASIS_loader, batch_size: int, epochs: int, training_history_location: str, image_sample_count:int = 5, image_sample_output:str = "output/", model_output:str = "model/") -> None:
    """
        Fits the model to a given set of training images. Upon conclusion of an epoch, the epoch's training history is saved to disk. If requested, sample images are also taken and the model's current state is saved to disk at the end of each epoch as well 
                (either functionality can be disabled by passing None as the repsective filepath). We save after every epoch to protect against disruption during training. the styleGAN class and this fit method can pick up training again from where it has been left off. 
                Training history is appended, without overwriting existing data.  

        The custom GAN training paradigm is as follows:
            - Take a batch of real sample images, and train the discriminator to report a high "realness" score when it sees such an image.
            - Generate an equivilantly sized batch of fake image using the generator and its current set of weights. 
                Train the discrimimator to report a low "realness" score upon seeing such images.
            - Train the generator to produce images that make the discriminator with its newly trained weights output a high "realness" score. 
                The generator must adapt weights as the discriminator has just been trained on its current configuration.
            
            The loss used to achieve this training is applying the softmax function to the output of the discriminator. 
            This function is strictly positive and assymtotes towards zero as the input becomes more negative. 
            Hence taking the negative of the result of the discriminator when trained on real images and applying softmax will yield lower loss the higher the 'realness' score provided.
            Contrariwise taking the result of the discriminator directly when trained on fake images and applying softmax will yield lower loss the lower the 'realness' score provided.
            To apply this loss to the generator, the composed system (discriminator(generator)) is trained on random inputs to yield the realness score and subsequent loss of the images generated by the generator.
            The assotiated gradient is then computed and applied on only the generator's parameters, considering the discriminator's parameters to be fixed. 
    Args:
    ...
    for e in range(epochs):
        ...
        for b in range(num_batches):
            ...
            with tf.GradientTape() as tape:
                ...
                gen_loss =  tf.math.softplus(-gen_logits) #We desire the generator to learn weights that make it output something that results in a 'real' result from the linked discriminator following immediately in the pipeline
...
def train(model: str = "model/", image_source: str = "images/", epochs: int = 10) -> None:
    ...
    If model is set to None, then a new styleGAN is instantiated and saved to a default location (model/)
    upon completion of training. The training paradigm employed is described in the fit method employed.

    After training epochs have concluded, a graph of the model's training history is produced. 
    Provided history.csv is present and contains the training history of the specified model's previous training epochs,
    the generated plot will cover the model's entire training history, from its initialisation forward.

    Args:
        model (str, optional): Folder containing the styleGAN model to train. Defaults to "model/".
        image_source (str, optional): Folder containing training images. Defaults to "images/".
    ...
    IMAGE_SAMPLE_FOLDER = "output/" #TODO this isn't generated
    COMPRESSION_SIZE = 32
