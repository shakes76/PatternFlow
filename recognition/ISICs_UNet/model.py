...
def unet(class_num, input_size=(256,256,3)):
    ...
    conv3 = Conv2D(INIT_FILTER * 4, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(pool2)
    conv3 = Conv2D(INIT_FILTER * 4, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(conv3)
    pool3 = MaxPooling2D(pool_size=(2, 2))(conv3)

    conv4 = Conv2D(INIT_FILTER * 8, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(pool3)
    conv4 = Conv2D(INIT_FILTER * 8, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(conv4)
    drop4 = Dropout(dropout)(conv4)
    pool4 = MaxPooling2D(pool_size=(2, 2))(drop4)

    conv5 = Conv2D(INIT_FILTER * 16, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(pool4)
    conv5 = Conv2D(INIT_FILTER * 16, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(conv5)
    drop5 = Dropout(dropout)(conv5)

    up6 = Conv2D(INIT_FILTER * 8, (2,2), activation = 'relu', padding = 'same', kernel_initializer=hn)(UpSampling2D(size = (2,2))(drop5))
    concat6 = concatenate([drop4,up6], axis = 3)
    conv6 = Conv2D(INIT_FILTER * 8, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(concat6)
    conv6 = Conv2D(INIT_FILTER * 8, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(conv6)

    up7 = Conv2D(INIT_FILTER * 4, (2,2), activation = 'relu', padding = 'same', kernel_initializer=hn)(UpSampling2D(size = (2,2))(conv6))
    concat7 = concatenate([conv3,up7], axis = 3)
    conv7 = Conv2D(INIT_FILTER * 4, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(concat7)
    conv7 = Conv2D(INIT_FILTER * 4, (3,3), activation = 'relu', padding = 'same', kernel_initializer=hn)(conv7)
