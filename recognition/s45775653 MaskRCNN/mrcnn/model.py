...
def conv_block(input_tensor, kernel_size, filters, stage, block,
    ...
    x = BatchNorm(name=bn_name_base + '2a')(x, training=train_bn)
    x = KL.Activation('relu')(x)
                # CUSTOM: MODIFIED ADD DROPOUT LAYER
    # x = KL.Dropout(.25)(x)
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    x = KL.Activation('relu')(x)
                # CUSTOM: MODIFIED ADD DROPOUT LAYER ~ Dropout 2D to drop chosen feature set appliad accross all channels
    x = KL.Dropout(.25)(x)
...
class DataGenerator(KU.Sequence):
    ...
    def __init__(self, dataset, config, shuffle=True, augmentation=None, # Original augmentation=None
...
class MaskRCNN(object):
    ...
    def build(self, mode, config):
        ...
        input_image = KL.Input(
            ...
            shape=[512, 512, config.IMAGE_SHAPE[2]], name="input_image")
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        callbacks = [
            keras.callbacks.TensorBoard(log_dir=self.log_dir,
                                        histogram_freq=0, write_graph=True, write_images=False), #changed histogram_freq to 1. default = 0
        ...
        self.keras_model.fit(
            ...
            callbacks=callbacks,
            # validation_split = 0.2, # Original setting does not include this line. delete when not working,
            validation_data=val_generator,
            validation_steps=self.config.VALIDATION_STEPS,
            max_queue_size=100,
            workers=workers,
            use_multiprocessing=workers > 1, #Original Setting
...
def mold_image(images, config):
    ...
    colors in RGB order.
    """
    images = images/255 # modified to normalize
    # return images.astype(np.float32) - config.MEAN_PIXEL # ORIGINAL IMPLEMENTATION
...
def unmold_image(normalized_images, config):
    ...
    # return (normalized_images + config.MEAN_PIXEL).astype(np.uint8) # ORIGINAL IMPLEMENTATION
    return normalied_images.astype(np.uint8)
