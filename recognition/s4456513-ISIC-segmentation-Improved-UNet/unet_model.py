...
def Improved_UNet(input_size):
    ...
    # four VGG structures
    # first VGG
    conv1 = Conv2D(filter_base, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(inputs)
    conv2 = Conv2D(filter_base, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(conv1)
    ...
    conv6 = Conv2D(filter_base * 4, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(conv5)
    ...
    # bottom 
    conv9 = Conv2D(filter_base * 16, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(pool4)
    ...
    conv11 = Conv2D(filter_base * 8, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(up4)
    merge4 = Concatenate(axis=3)([conv8, up4])
    conv12 = Conv2D(filter_base * 8, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(merge4)
    conv13 = Conv2D(filter_base * 8, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(conv12)

    # second CONCAT
    up3 = UpSampling2D((2,2), interpolation='nearest')(conv13)
    conv14 = Conv2D(filter_base * 4, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(up3)
    merge3 = Concatenate(axis=3)([conv6, conv14])
    conv15 = Conv2D(filter_base * 4, 3, activation='relu', padding = 'same', kernel_initializer = 'he_normal')(merge3)
