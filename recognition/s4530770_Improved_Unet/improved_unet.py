...
def localisation_module(input_layer, context, filters):
    ...
    seg = norm
    return Conv2D(filters/2, 1, strides=1, padding="same", activation="relu")(norm), seg
...
def build_model(input_size, filter_size):
    ...
    out, dump = localisation_module(out, context_4, filter_size*8)
    out = upsample_module(out, filter_size*4)

    # Level 2 localistaion pathway
    out, seg_1 = localisation_module(out, context_3, filter_size*4)
    out = upsample_module(out, filter_size*2)

    # Level 3 localisation pathway
    out, seg_2 = localisation_module(out, context_2, filter_size*2)
    out = upsample_module(out, filter_size)

    # Level 4 localisation pathway
    # Use seg output as new layer as we don't want a 1x1 convolution for the output
    dump, out = localisation_module(out, context_1, filter_size)
