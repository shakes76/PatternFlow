...
if not os.path.exists("pixelcnn.ckpt") or improve_existing:
    ...
    encoded = tf.reshape(model.encoder.predict(
        data["train"][0:len(data["train"])//4]), shape=(-1, latent_dims))
    indices = get_indices(
        model.vq.variables[0], encoded, quantize=False, splits=1)
    ...
    pixelcnn.fit(
        x=indices,
        y=tf.cast(tf.one_hot(tf.cast(indices, dtype=tf.int64),
...
if not os.path.exists("pixelcnn.ckpt") or improve_existing:
    pixelcnn.predict(tf.random.uniform(shape=(1, 32, 32),
...
for row in range(32):
    ...
    for col in range(32):
        ...
        generated[:, row, col] = tf.reshape(
            tf.random.categorical(probabilities, 1), shape=(4,))
...
for i in range(n):
    ...
    pred = model.decoder.predict(tf.reshape(tf.gather(model.vq.variables[0], tf.cast(
        generated[i], dtype=tf.int64)), shape=(1, 32, 32, latent_dims)))
